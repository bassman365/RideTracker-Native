...
import { getShortDisplayDate, getDisplayTimeFromSeconds, refreshProgramCollections } from '../../lib/helpers';
...
export default class ViewRidesScreen extends Component {
  ...
  darkTextPreferred(backgroundColorHex) {
    const color = (backgroundColorHex.charAt(0) === '#')
  ...
  async getRidesAsync() {
    try {
      ...
      if (!programCollections || !programCollections.length > 0) {
        await refreshProgramCollections();
  ...
  render() {
    return (
      <FlatGrid
        ...
        renderItem={ride => (
          <View style={[styles.itemContainer, { backgroundColor: ride.item.color ? ride.item.color : '#2980b9' }]}>
            <Text style={ride.item.textStyle}>Program: {ride.item.program}</Text>
            <Text style={ride.item.textStyle}>Date: {getShortDisplayDate(ride.item.date)}</Text>
            <Text style={ride.item.textStyle}>Weight: {ride.item.weight}</Text>
            <Text style={ride.item.textStyle}>Calories: {ride.item.calories}</Text>
            <Text style={ride.item.textStyle}>Distance: {ride.item.distance}</Text>
            <Text style={ride.item.textStyle}>Level: {ride.item.level}</Text>
            <Text style={ride.item.textStyle}>Duration: {getDisplayTimeFromSeconds(ride.item.durationSeconds)}</Text>
          </View>
        )}
